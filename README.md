...
│   └── vexflow.js                # 乐谱渲染引擎
├── checkpoints/                  # 模型权重（需从 Release 下载）
...
由于文件大小限制，以下文件需要从 [GitHub Releases](../../releases/tag/v8.0) 下载：
...
项目已包含 `lilypond-2.24.4/` 目录，无需额外安装。
...
启动后会看到 PianoScribe 启动动画（白色背景，图标掉落+花朵绽放），然后进入主界面。
...
4. **查看乐谱** - 切换到"乐谱"标签页查看生成的乐谱
5. **播放** - 切换到"播放"标签页，可切换原始力度/统一音量模式
6. **难度分析** - 切换到"难度"标签页查看曲目难度评级
...
预编译安装包和大文件请前往 [GitHub Releases](../../releases/tag/v8.0) 页面下载。
