...
# 创建虚拟环境
conda create -n pianoscribe python=3.10
...
- **操作系统**: Windows 10 / 11（64位）。macOS / Linux 理论上可运行，尚未测试。
- **Python**: 3.10 或 3.11
...
| Windows 10/11 | 完全支持 |
| macOS | 理论兼容，待测试 |
| Ubuntu/Linux | 理论兼容，待测试 |

核心代码均为跨平台 Python，仅在 FluidSynth DLL 加载和 MIDI 回退播放 (MCI) 使用了 Windows API。macOS/Linux 可使用系统包管理器安装 FluidSynth（`brew install fluidsynth` / `apt install fluidsynth`），应用即可正常运行。
